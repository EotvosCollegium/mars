...
<blockquote>A tranzakció a rendszergazdai kasszába fog kerülni.</blockquote>
<div class="row">
<form method="POST" action="{{ route('print-account.update') }}" @if(!auth()->user()->isAdmin()) onsubmit="return confirm('Biztosan szeretnéd az egyenleget módosítani?')" @endif>
        @csrf
        @method('PUT')
